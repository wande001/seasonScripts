...
modelS = ["ESP","CanCM3", "CanCM4", "FLOR","CCSM"]
precVarNameS = "prec"
tempVarName = "tas"
  ...
  for r in range(len(refModelS)):
    ...
    for year in range(beginYear,endYear+1):
      for month in range(1,13):
        ...
        if model == "ESP":
          job.writelines("python /tigress/nwanders/Scripts/hydroSeasonal/seasonScripts/runEPSVIC.py %s %s %s %s" %(startTime, endTime, model, refModel))
        else:
