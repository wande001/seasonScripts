...
ncOutputFile = "/tigress/nwanders/Scripts/hydroSeasonal/resultsNetCDF/PCRGLOBWB_"+ref+"_"+varOutPutName+"_lag_"+str(lag)+"_Coef_Catchment.nc4"
...
varNames = ["CanCM3","CanCM4","FLOR", "correlation","signif", "bias", "RMSE"]
varUnits = ["-","-","-","-","-","m3/s","m3/s"]
MV = -999.
...
for event in range(0,end,step):
    ...
    weightNMME = np.zeros((30,360,720))

    for i in range(360):
      print i
      for j in range(720):
        matrixData = np.vstack((ensCanCM3[:,i,j], ensCanCM4[:,i,j], ensFLOR[:,i,j])).transpose()
        #print matrixData.shape
        #print dataPGF[:,i,j]
        #print scipy.optimize.nnls(matrixData, dataPGF[:,i,j])
        out[:,i,j] = scipy.optimize.nnls(matrixData, dataPGF[:,i,j])[0]
        weightNMME[:,i,j] = np.sum(out[:,i,j]*matrixData, axis=1)
    ...
    data2NetCDF(ncOutputFile, "CanCM3", out[0,:,:], lagToDateTime(dateInput, 0, model), posCnt = posCount)
    data2NetCDF(ncOutputFile, "CanCM4", out[1,:,:], lagToDateTime(dateInput, 0, model), posCnt = posCount)
    data2NetCDF(ncOutputFile, "FLOR", out[2,:,:], lagToDateTime(dateInput, 0, model), posCnt = posCount)
